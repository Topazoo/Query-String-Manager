...
class TestParseQueryString(unittest.TestCase):
    """
        Tests for :class:`QueryStringParser._parse_raw_query_string()`
    """
    ...
    def test_throws_exception_on_invalid_query_string(self):
        ...
        for test_dict in TEST_INVALID_STRINGS:
            self.assertRaises(ValueError, lambda: QueryStringParser._parse_raw_query_string(test_dict))
    ...
    def test_parse_single_arg_query_string(self):
        ...
        for test_dict in TEST_DICTS_AND_RESULTS:
            self.assertEquals(test_dict[0], QueryStringParser._parse_raw_query_string(test_dict[1]))
    ...
    def test_not_normalizing_values(self):
        ...
        for test_dict in TEST_DICTS_AND_RESULTS:
            self.assertEquals(test_dict[0], QueryStringParser._parse_raw_query_string(test_dict[1], False))
